...
ints$ints_exp = exp(ints$intercept)
slopes = nestZs%>%select(Site,slope)
cz_data =  as.data.frame(cbind(ints[,1:3],slopes[,2]))
colnames(cz_data) = c("Site","int","c","z")
site_data_ints = inner_join(site_data,cz_data)
site_data_ints$cz = site_data_ints$c*site_data_ints$z
...
meanPHI = round(mean(site_data_ints$Pos_Hetero_Index, na.rm = TRUE),2)
x = site_data_ints$Pos_Hetero_Index
x[is.na(x)] = meanPHI
site_data_ints$Pos_Hetero_Index = x
...
x = site_data_ints$c
y = site_data_ints$z
correl = round(cor(site_data_ints$c, site_data_ints$z, method = "spearman"),1)
g1 = ggplot(site_data_ints, aes(x = c, y = z))+
  geom_point()+
...
g2 = ggplot(site_data_ints, aes(x = c, y = Richness ))+
  geom_point()+
  geom_smooth(method = lm)+
  xlab("c")
 

g3 = ggplot(site_data_ints, aes(x = z, y = Richness ))+
  geom_point()+
  geom_smooth(method = lm)+
  xlab("z")

g4 = ggplot(site_data_ints, aes(x = cz, y = Richness))+
  geom_point()+
  geom_smooth(method = lm)+
  xlab("cz")
...
subset_all = site_data_ints%>%select("Site","Area_ha",
...
colnames(subset_all) = c("Site","Area",
                         "Northing", "PHI","Buffer",
                         "no_MSG", "no_NVC","sd_pH","sd_SOM","sd_LBA",
...
## ps changed so outliers are now IN
subset_sd = subset_all%>%select("Site",
                               "Northing", "PHI","Buffer",
                               "no_MSG", "no_NVC","sd_pH","sd_SOM","sd_LBA",
                               "sd_meandbh","sd_TD","area_ratio","c")



subset_mean = subset_all%>%select("Site",
...
rescaled_mean_data = apply(data[,-12],2, rescale)
rescaled_mean_data = as.data.frame(cbind(c, rescaled_mean_data))
```


```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
#create the model
mod_mean = lm(c ~., data=rescaled_mean_data, na.action = "na.fail")
```
...
#rescale the data
library(arm) #for standarize
rescaled_sd_data = apply(data[,-12],2, rescale)
rescaled_sd_data = as.data.frame(cbind(c, rescaled_sd_data))
```


```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
#create the model
mod_sd = lm(c~., data=rescaled_sd_data, na.action = "na.fail")
```
...
empirical_int =  site_data_ints$c
