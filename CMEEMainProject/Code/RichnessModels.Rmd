...
The graph shows the averaged effect sizes of the model with delta < 2. The effect which have a CI which does not include zero and therefore may influence richness [Gruber] are mean soil organic matter, number of NVC codes, and Positive Heterogeneity Index. Because the data is standardized, the effects sizes can be directly compared as a unit of one represents two standard deviations of change in response. Therefore the number of NVC codes can be seen to have the greatest effect, with SOM and PHI having approximately equal but opposite effects.
